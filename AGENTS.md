...
WikiHub 시스템 설계·개발·배포를 위한 최상위 거버넌스. **OpenCode** · **Hermes** (외부 오케스트레이션) 가 본 파일을 정본으로 참조. 운영 환경(live wikihub instance)에서 위키를 다루는 agent는 [`_system/wiki-schema.md`](_system/wiki-schema.md) (Operator Guide)로 전환.
...
## 3. 5-step issue-first workflow

**플로우**: Plan → Analysis & Design → Implementation → (Review) → (Deploy) → archive. Step 4·5 는 조건부 생략 가능 (생략 결정은 Step 1 plan.md 에 사전 선언). 분기 base = **`origin/<현재 버전 브랜치>`** (예: `origin/v0.1.12`) — main 직접 분기·commit 금지. 버전 브랜치는 release 시 `release.sh`가 cleanup 하므로, 다음 사이클 시작은 첫 issue PR 시 Hermes `github-dev-flow` Step 4 가 `scripts/bootstrap_version.sh` 를 자동 호출한다. **메서드론 정본은 본 §3**, 실무 how-to / git 액션 5단계 / 리뷰 규칙 / HISTORY.md 형식은 [agent_dev_guide §3](docs/agent_dev_guide.md#5단계-issue-first-workflow) 참조.
...
**다음 사이클 시작** (release 직후 첫 issue 작업 시):
- Hermes `github-dev-flow` Step 4 가 `origin/{base_branch}` 부재를 감지 → `scripts/bootstrap_version.sh` 자동 호출
...
병렬 작업(서브에이전트 / cmux 패널) 시 필수. issue 분기 base = `origin/<현재 버전 브랜치>`. 패턴 + 명령: [agent_dev_guide §Worktree](docs/agent_dev_guide.md#git-worktree-활용).
...
- **Gitignored (절대 commit 금지)**: `.claude/`, `.agents/`, `.hermes/`, `skills-lock.json`, `.env`, `wikihub.yaml`, `.credentials/`, `*token_*.json` — 로컬 세션 산출물·자격증명. **`.opencode/`는 repo 추적 대상** (ADR-0046).
...
- **세션 시작 권장**: "AGENTS.md 를 읽고 현재 작업 컨텍스트에 맞춰 적용해줘" + 관련 docs/adr 명시. Issue 작업은 Hermes의 `github-dev-flow` 스킬을 통해 처리하며, OpenCode는 oneshot 구현·리뷰 호출만 수행.
