...
- **운영 로컬 helper 가 lint 판정을 돌리는 구조** — 실행 계층(`lint_mechanical` · `analyze_graph_v3` · `rebuild_index` · `step7_apply` · `apply_fixes`) 과 계상·진단 보조 2종이 repo 밖(`$WIKIHUB_HOME/_scripts/`)에 있어 git 이 추적하지 못하고 다른 호스트로 이식하면 lint 가 절반만 동작하던 문제. `scripts/_helpers/` 로 정본화하고 경로 복원 체계(`--wiki-home`/`--src` arg > `$WIKIHUB_HOME`/`$WIKIHUB_SRC` > `$WIKIHUB_YAML` 부모 > ADR-0034 default)를 도입해 `/home/ubuntu/wikihub` 하드코딩을 제거 (#201 ⑥, PR #205)
