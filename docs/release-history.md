...
## [2026-09-22] v0.1.18 — 정본 helper 이원화 해소

- **목적**: v0.1.17 릴리즈 직후 첫 lint 회차(13:52 KST) report 가 정본 수치와 두 자릿수 차이나는 값을 함께 실은 문제를 해소한다. 원인은 v0.1.17 의 PR #205 가 사장 스크립트를 정본으로 등재한 것으로, 발견 경위는 lint report 의 "개발 소관 ⑦-a" 추적이다 (#206).
- **로직**: `link_audit_v2.py` 는 호출자·소비처가 0건인 사장 스크립트였고(산출물 4 키 전량 참조 0건), stale 한 `_lint/alias_index.json`(mtime 2026-09-01) 을 입력으로 읽어 fence 미스트립·카테고리 오판으로 정본과 두 자릿수 차이(위반 1 = 153 vs 22, 위반 2 = 60 vs 0)를 냈다. 정본에서 제거하고 Step 2 스캔을 `_wl_step2_spec.py` 단일 경로로 유지한다.
- **트레이드오프**:
  - PR #207 이 `main` 에 직접 머지됐다 — v0.1.17 릴리즈로 버전 브랜치가 삭제된 직후라 정상 경로(버전 브랜치 → release.sh)를 쓸 수 없었다. 본 릴리즈에서 `bootstrap_version.sh` 로 v0.1.18 브랜치를 정상 생성해 사이클을 복귀시킨다.
  - #206 은 지사서가 **자기 작업(PR #205)의 결함**을 신고한 것이다 — v0.1.17 릴리즈에 포함된 결함이므로 본 릴리즈에서 정정한다.
- **결론**: main merge + annotated tag `v0.1.18` + `latest` (2026-09-22). 운영자 `install.sh --branch latest`.
- **참조**: `docs/changelog.md` [v0.1.18]. 관련 open issue — #149(데이터 백업), #116(PDF 추출), #201(DoD 6 대기).

---

## [2026-09-22] v0.1.19 — 검출기 정합 6건 (참조·로그·파서)

- **목적**: lint 가 매 회차 보고하던 "개발 소관" 잔여를 일괄 정리하고, 검출기가 자기 규정을 지키지 못해 수개월 누적된 결함을 코드·검출기 양쪽에서 닫는다. 6개 PR 을 한 사이클로 묶었다 (#208·#210·#211·#213·#214·#215).
- **로직**:
  - **참조 매칭 3중 분기 (#215)** — 이름 유형별로 규칙을 나눈다. ① 확장자 포함 → 완전 일치, ② ASCII → 토큰 경계(`re.I` 필수), ③ 한글·CJK → 표기변형 정규화 후 **같은 줄** substring. 전역 정규화는 개행까지 지워 인접 단어를 이어붙여 오탐을 **위조**하므로 금지. `ref_match.py`(판정 정본) · `ref_ledger.py`(등록 근거 원장) · `ref_audit.py`(전수 검출기).
  - **중복 전수 검출 (#210)** — `detect_alias_duplicates.py` 가 `referenced_by` 를 검사하지 않아 lint 가 편입 등록 1페이지만 보고했다(실제 362페이지 — 181배 격차). `ref_audit.py` + lint Step 4 배선.
  - **Step 6 후보 교정 (#214)** — 후보 13건 중 11건이 코드 펜스 내부였다. 펜스 밖 본문 헤딩만 취하도록 하고 CommonMark 정합(닫는 펜스 뒤 공백/탭만, ATX 3칸 한계, 닫는 `#` 시퀀스 앞 공백 조건)을 맞췄다. 후보 키 lowercase 통일로 표기 변형 분리 집계(→ 후보 소실)를 막았다.
  - **report 한자 살균 (#213)** — lint 가 결함을 보고하면서 그 한자를 그대로 인용해 report 자신이 다음 회차 검출 대상이 됐다. `U+XXXX` 표기·개수 요약만 쓰게 했다.
  - **log.md 무결성 (#211)** — append-only 이력의 서식을 검출한다(역행·접두·헤더형식·Trigger). **역행은 자동 수정하지 않는다** — 원래 시각을 알 수 없다. `ingest.md` Step 5 에 자기 검증 규약 추가.
  - **aliases 파서 (#208)** — 구 정규식 `^\s+-\s+` 가 column-0 block 을 전량 미인식(실측: column 0 이 2,020 / 들여쓰기 272 — 다수 누락). `^\s*-\s+` + top-level 키 경계로 교정.
  - **flock 세션 소유 wrapper (#201 ④)** — Hermes tool 호출마다 단명 subprocess 라 fd 상속 주체가 없는 문제. `wl_guarded.sh` + systemd unit 배선.
- **트레이드오프**:
  - 오등록·중복 **데이터 정리**는 운영 소관이다 — 진성분 보존 판정이 필요하다. v0.1.19 는 코드·검출기까지다.
  - `log_integrity` 의 역행 8건은 **미조치로 남긴다** — 원래 시각을 복원할 근거가 없다.
  - `#211` DoD 5번(다음 lint 회차 report 등장)은 운영 반영 후에만 확인된다.
  - 리뷰는 `glm-5.3` + 추론 억제 블록으로 돌렸다 — 억제 없이는 `reason=length`(출력 32K 소진)로 보고를 하나도 내지 못했다.
- **결론**: main merge + annotated tag `v0.1.19` + `latest` (2026-09-22). 운영자 `install.sh --branch latest`.
- **참조**: `docs/changelog.md` [v0.1.19]. 관련 open issue — #149(데이터 백업), #116(PDF 추출 방법론).
