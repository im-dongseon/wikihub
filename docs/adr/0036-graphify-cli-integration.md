...
- 2026-05-26: `graphify_path_absolute` (v0.1.10) 가 wh-lint playbook 의 `graphify-out/graph.json` 상대 경로 5건을 `$WIKIHUB_HOME/graphify-out/graph.json` 절대 경로로 정정 + Step 7 에 stale `wiki/graphify-out/` cleanup 책임 신규. OCI 운영 중 발견 — wh-lint skill (LLM) 의 CWD context drift 로 상대 경로가 `$WIKIHUB_HOME/wiki/graphify-out/` 으로 잘못 resolution → stale 934-node graph 읽고 0-edges 회귀. 진단 가이드 `docs/references/graph-path-resolution.md` 신설. `.graphifyignore` 에 `graphify-out/` 추가 (defense-in-depth — graphify CLI 의 잘못된 `--out` 호출 시 자기 출력 재scan 차단). ADR 결정 변경 없음 (implementation hardening).
- 2026-05-27 (graphifyignore_migration follow-up): 다중 canary 검증에서 surface — `cp -n` (template 배치) 는 fresh install 만 효과 → 기존 instance 의 `wiki/.graphifyignore` 는 `graphify-out/` line 미보유 (defense-in-depth layer 3 미적용). install.sh `_migrate_graphifyignore` 추가 — `^graphify-out/?$` regex 부재 시만 idempotent append (운영자 customization 보존). `_step5_instance_dirs` 후속 hook. 본 fn 가 매 install (update + fresh) 시점 작동 → 기존 instance 의 graphify-out/ ignore line 자동 보강.
- 2026-05-28: `profile_timeout_override` (v0.1.10, Issue #36) 가 `operations.graphify_profiles.<profile>.timeout_sec` profile-level timeout override 추가. 우선순위: profile-specific → `graphify_timeout_sec` → 900 default. yq 3단 fallback chain (`scripts/wikihub_graphify.sh:73`). 기존 동작 100% 보존 (`graphify_profiles` 미설정 시 fallback).
