...
- `operations`: `lint_interval_hours`·`lint_contradiction_check`·`graphify_enabled`·`max_concurrent_vaults`·`retry.*`·`disk.*`·`fatal_webhook_url`·`fatal_webhook_timeout_sec`·`instance_label`·`rclone_min_version`·`rclone_max_version`·`vfs_cache_max_size`·`vfs_refresh_mode`·`graphify_min_version`·`graphify_max_version`·`graphify_backend`
...
- **graphify env file 검증** (ADR-0036 + §Note 2026-05-20): `~/.config/wikihub/env` 파일 존재 (install.sh `_step5_instance_dirs` 가 보장) + 권한 0600. **내용 자체는 검증 안 함** — backend (yaml `operations.graphify_backend`) 마다 필요 env 가 다름 + Hermes 측에 이미 설정됐을 수 있음. 운영자 책임.
  - **Hermes terminal.env_passthrough 안내** (ADR-0036 §Note 2026-05-20): wh-lint Step 9 의 graphify 는 hermes 의 terminal tool 로 spawn 됨. Hermes 의 tirith 가 default 로 secret env 를 subprocess 에서 strip — `~/.hermes/config.yaml` 의 `terminal.env_passthrough` 에 backend env (예: `OLLAMA_API_KEY`, `OLLAMA_BASE_URL`, `OLLAMA_MODEL`) 추가 필요. 안내만 (자동 갱신 안 함 — Hermes config 는 wikihub spec 외부).
