...
- **한자 (漢字) 감지 시 한글로 변환** — deepseek-v4.1-flash (구 v4-pro) / minimax 등 일부 모델이 동음이의 한국어를 한자 표기로 출력하는 결함 (Hermes OCI 실증, 2026-05-20, lint.md 와 동일). 예: "기획(企劃)" → "기획"; "권한(權限)" → "권한". 고유명사 (인명·지명·조직명 중 한국 외 출처) 는 예외 허용.
